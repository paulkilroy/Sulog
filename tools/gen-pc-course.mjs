/* Turn the Gemini-extracted PC blocks (scratchpad/pc-blocks.json) into docs/schema/pc-course.sql.
   ...
   Run: node tools/gen-pc-course.mjs <path-to-pc-blocks.json>  */
import fs from "fs";
...
function repairExpr(war, en) {
  ...
  en = en.replace(/^([a-z][a-z ]{1,24})\/([A-Z].*)$/, (m, noun, owner) => `${owner}'s ${noun.trim()}`);
...
if (pruned.length) {
  ...
  console.error(`\n⚠️  BUILD WARNING — pruned ${pruned.length} empty ${pruned.length === 1 ? "section" : "sections"} (source had items but none survived — blank answer side, all rejected as FABRICATED, or all proximity-dropped as not-yet-taught):`);
...
const out = [];
out.push("-- GENERATED by tools/gen-pc-course.mjs from Gemini-extracted PC blocks. AI-extracted from");
...
fs.writeFileSync("docs/schema/pc-course.sql", out.join("\n\n") + "\n");
const paraCt = blocks.filter((b) => b.type === "vocab" && b.title).length;
console.log(`✓ pc-course.sql — ${lessons.length} lessons · dictionary +${dict.size} · expressions +${exprRows.length} · blocks +${blocks.length} (${paraCt} paradigm) · items +${items.length}`);
