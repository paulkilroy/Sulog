...
fs.writeFileSync(path.join(root, "docs/courses/frequency/mc-request.txt"), out);
const words = out.split(/\s+/).length;
console.log(`wrote docs/courses/frequency/mc-request.txt — ${STORIES.length} stories, ~${words.toLocaleString()} words, ${out.length.toLocaleString()} chars`);
