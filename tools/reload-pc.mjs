#!/usr/bin/env node
/* Ingestion: reload the Peace Corps course into the live DB from docs/schema/pc-course.sql,
 * then regenerate the "Course vs. Book" verification page so it always reflects what's live.
 *
 * Pipeline:  node tools/gen-pc-course.mjs      # rebuild pc-course.sql from the Gemini blocks
...
// ---- close the review loop, inbound: fold Review-Queue-approved exercise answers into the
// course source, then regenerate the seed so this reload ships them.
console.log("→ harvest: folding queue-approved answers into the course source");
execSync("node tools/harvest-approvals.mjs", { stdio: "inherit", env: process.env });
console.log("→ regenerating pc-course.sql (picks up any harvested answers)");
execSync("node tools/gen-pc-course.mjs", { stdio: "inherit", env: process.env });

const seed = fs.readFileSync("docs/schema/pc-course.sql", "utf8");
for (const marker of ["insert into lessons", "insert into block_items", "'pc-l1a'", "GENERATED by tools/gen-pc-course.mjs"])
  if (!seed.includes(marker)) { console.error(`✗ pc-course.sql looks broken/truncated (missing "${marker}") — refusing to reload.`); process.exit(1); }
...
try {
  ...
  await c.query(seed); // fresh pc-course.sql
...
console.log("\n[2/4] homograph senses (meaning-overrides)…");
execSync("node tools/sync-meaning-overrides.mjs", { stdio: "inherit", env: process.env });
...
// ---- close the review loop, outbound: file the rebuilt course's open questions into the queue
// (idempotent — answered/resolved rows are never touched or resurrected)
console.log("→ emitting build issues into the review queue");
execSync("node tools/emit-build-issues.mjs", { stdio: "inherit", env: process.env });
