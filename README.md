...
### Client-side data maps (NOT in the DB)

Beyond the relational tables, the app ships hand-authored / generated **lookup maps** as JS modules
under `src/courses/waray/`. These aren't in `schema.sql` because they live in the bundle, not the
database — but they're part of the data model. The reader's tap-to-define and the topic/badge
displays all read from here:

| Map | Lives in | What it is | Fed by |
|---|---|---|---|
| **`DEFINITIONS`** | `stories.js` | word → English definition — the **reader's tap-to-define source** (a few thousand entries: function words + course/story vocab). *Not* the 25k DB dictionary. | generated by `tools/build-stories.mjs` from the lexicon + Tramp + `DEFINITION_FIX` |
| **`DEFINITION_FIX`** | `variants.js` | curated definitions that **win** over Tramp/lexicon (loanwords, function words a 1991 dict lacks) | hand-authored |
| **`VARIANTS`** | `variants.js` | spelling/dialect/OCR variant → the canonical taught word | hand-authored |
| **`CHUNKS`** | `variants.js` | multi-word idiom → one definition (reader does longest-match) | hand-authored |
| **`DIALECT_FORMS` / `DIALECT_PRESETS`** | `variants.js` | regional variant toggles in the Language door | hand-authored |
| **`STORIES`** | `stories.js` | reader texts (paragraphs + comprehension questions) | generated |
| **`TOPIC_META` / `TOPICS`** | `sulog.jsx` | topic id → display label (the card tag) | hand-authored + derived |
| **`TAUGHT_IN`** | `sulog.jsx` | word → first lesson that teaches it (the "Taught in Lesson X" badge) | derived from the curriculum |
| **`MARKER_DEFINITIONS` / `CASE_MARKERS`** | `sulog.jsx` | case-marker → role (the marker drill) | hand-authored |
| `ADDED_WORDS`, `MINED_BY_UNIT`, `CONFIRM_CANDIDATES`, `ELLA_QUESTIONS` | `src/courses/waray/` | curriculum gap-fills, mined phrases, review-queue data | mixed |

**The one gap to know:** `DEFINITIONS` (reader tap) and the DB `dictionary` (search) are *separate*
sources — the reader map has ~thousands, the DB has ~25k. Tapping a rare word in a story can say
"not in the dictionary" even though the Dictionary *search* finds it. Unifying them (reader falls
back to the DB dictionary) is the open "tap-to-define everywhere" item.
