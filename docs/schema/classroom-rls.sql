...
create or replace function join_class(p_code text) returns text language plpgsql security definer as $$
declare cid text;
begin
...
create policy classes_read       on classes for select to authenticated using (
  ...
  or exists (select 1 from enrollments e where e.class_id = classes.id and e.student_id = auth.uid()));
...
create policy enroll_instructor on enrollments for select to authenticated using (
  is_admin() or exists (select 1 from classes c where c.id = enrollments.class_id and c.instructor_id = auth.uid()));
...
create policy fb_instructor_read on feedback for select to authenticated using (
  class_id is not null and exists (select 1 from classes c where c.id = feedback.class_id and c.instructor_id = auth.uid()));
...
do $$ declare t text; begin
  foreach t in array array['progress','lesson_progress','unit_progress','user_streak'] loop
    ...
    execute format($f$create policy %I on %I for select to authenticated using (
      exists (select 1 from enrollments e join classes c on c.id = e.class_id
              where e.student_id = %I.user_id and c.instructor_id = auth.uid()))$f$, t||'_instructor_read', t, t);
  end loop;
end $$;
