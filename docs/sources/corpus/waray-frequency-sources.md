...
- `docs/sources/waray-lexicon.json` — **the normalized source of truth.** Entities +
  links: `sources`, `lexemes` (1,157 — waray, pos, gloss, freq, tier, top1000, inDeck,
  cardId), `cards` (628 — SEED mirror; phrases link to component lexemes), `sentences`
  (1,562 — each linked to the lexemes it contains). Generated by the tool; markdown docs
  are views of it. JSON now; promote to SQLite if/when the joins demand it.
